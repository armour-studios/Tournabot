  ...
  execute(message, client) {
    ...
    function runResults() {
      ...
      queryAPI(query, { slug }).then(data => {
        ...
        let images = data.data.user.images;
        if (images[0]) {
          imageurl = images[0].url;
          if (images[1]) {
            if ((images[0].height > images[1].height) && (images[0].width > images[1].width)) imageurl = images[1].url;
            if ((images[0].width - images[0].height) > (images[1].width - images[1].height)) imageurl = images[1].url;
          }
        }
        ...
        function queryRecurseAndSend() {
          ...
          queryAPI(query, { page, slug, playerIds }).then(data => {
                            ...
                            if (sets[tIndex].join('\n').length + 13 >= setsCharLength) {
                              ...
                              function doubleCheckCount() {
                                if (sets[tIndex].join('\n').length > 1000) {
                                  ...
                                  if (sets[tIndex][sets[tIndex].length - 1].startsWith('*For ')) sets[tIndex].pop();

                                  if (sets[tIndex][sets[tIndex].length - 1] === '`And more...`') sets[tIndex].pop();
